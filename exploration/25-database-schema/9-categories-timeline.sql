...
select name_category,time_bin, sum(sum) 
from (select time_bin,sum, name_category 
      from(select * 
      ...
      left join 
      (select id_group, name_category from groups) as boo 
      using (id_group)) as koo 
group by name_category, time_bin;